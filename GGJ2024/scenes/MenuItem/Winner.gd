...
func go(nb):
	winner = nb
	$Labels/Label.text ="Player "+str(nb+1) 
	...
	tw.tween_property($Labels, "modulate:a", 1.0, 0.2).set_ease(Tween.EASE_OUT).set_trans(Tween.TRANS_SINE)
	tw.tween_callback(_on_tween_ended)

func _on_tween_ended():
	Globals.restart_game(winner)
